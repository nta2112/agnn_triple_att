...
class CustomImageFolder(data.Dataset):
    def __init__(self, root, partition='train', category='custom', image_size=84, split_path=None):
        ...
        # Cấu hình transformer scale đa kích thước
        ...
        if os.path.exists(split_path_to_use):
            ...
            base_dir = os.path.join(self.root, partition)
            if not os.path.exists(base_dir):
                # Fallback to flat root directory if no partition train/val subfolders exist
                base_dir = self.root
